...
function handleInput_1(lines, first = true) {
    ...
    let boards = [];
    lodash_1._.range(2, lines.length, 6).forEach(n => {
    ...
    boards.forEach((board, n) => {
        ...
        Object.keys(boardNumbersData).forEach(number => {
            if (!Object.keys(numbers).includes(number))
                numbers[number] = [];
    ...
    bingoNumbers.forEach(number => {
        if (first && winningBoard !== undefined)
            return;
        if (!Object.keys(numbers).includes(number))
            return;
        if (nbWinningBoard === boards.length)
            return;
...
function getBoard(boardlines) {
    const lines = boardlines.map(line => lodash_1._.range(0, line.length, 3).map(n => Number(line.slice(n, n + 2))));
    let board = {
        lines,
        columns: lodash_1._.range(0, lines[0].length).map(n => lines.map(line => line[n]))
    };
    return board;
}
